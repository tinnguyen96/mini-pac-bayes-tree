################################################################################
# Author:    Tin Nguyen
# Name:  ADDT_nS_master.py 
# Most recent update: 05/14/2018
# 
...
class DDT:
    ## Input: 
    def __init__(self, numOfLabels, penaltyType, priorType, alterType, val_ratio=0.35, 
    ...
    def fit(self, X, y, random_state=42):
        ...
        # Different executions of the alternate minimization 
        if (self._alterType == "jointErrFirst"):
            self.__logSearch_joint()
            self.__linearSearch_errLambda()
            self.__linearSearch_penLambda()
        elif (self._alterType == "jointPenFirst"):
            self.__logSearch_joint()
            self.__linearSearch_penLambda()
            self.__linearSearch_errLambda()
        elif (self._alterType == "joint"):
            self.__logSearch_joint()
            self.__linearSearch_joint()
        elif (self._alterType == "penFirst"):
            self.__logSearch_penLambda(True)
            self.__linearSearch_penLambda()
            self.__logSearch_errLambda(False)
            self.__linearSearch_errLambda()
        elif (self._alterType == "errFirst"):
            self.__logSearch_errLambda(True)
            self.__linearSearch_errLambda()
            self.__logSearch_penLambda(False)
            self.__linearSearch_penLambda()
        elif (self._alterType == "mixedPenFirst"):
            self.__logSearch_penLambda(True)
            self.__logSearch_errLambda(False)
            self.__linearSearch_penLambda()
            self.__linearSearch_errLambda()
        elif (self._alterType == "mixedErrFirst"):
            self.__logSearch_errLambda(True)
            self.__logSearch_penLambda(False)
            self.__linearSearch_errLambda()
            self.__linearSearch_penLambda()
    ...
    def __logSearch_joint(self):
        ...
        if (self._report_lambda):
            print "Lambda range after joint search for err and penalty-lambda %s" %str(self._pen_lambda_range)
            print "Validation errors after joint search for err and penalty-lambda  %s" %str(self._val_err)
            print "Value of best joint lambda parameter %s" %str(self._best_pen_lambda)

    def __linearSearch_joint(self):
        # construct template tree
    ...
    def __update_penalty(self, node):
        ...
        if (exCount > 0):
            ...
            if (self._penaltyType == "ada"):
                codeLength = node.get_depth()
                temp = (codeLength)/float(self._trainSize)
                pHat = 4*np.max([probMass, temp])
                penalty = np.sqrt(2*pHat*temp)
            elif (self._penaltyType == "leakyAda"):
                penalty = np.sqrt(probMass/self._trainSize)
            elif (self._penaltyType == "bernstein"):
                penalty = probMass*node.get_depth()/float(exCount)
            elif (self._penaltyType == "fullBernstein"):
                pseudoExcess = node.get_empError()/probMass
                penalty = probMass*(np.sqrt(node.get_depth()*pseudoExcess/float(exCount))+node.get_depth()/float(exCount))
            elif (self._penaltyType == "none"):
                penalty = 0
        else:
            penalty = 0
    ...
    def __update_prior(self, node):
        if (self._priorType == "effInt"):
            exCount = np.sum(node.get_trainInfo())
            if (exCount > 0):
                prior = np.exp(-1)
            else:
                prior = 1 
        elif (self._priorType == "uniform"):
            prior = 1
