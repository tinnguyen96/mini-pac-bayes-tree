...
import ADDT_nS_master as aggregate

ex_index = int(sys.argv[1])
train_index = int(sys.argv[2])
penalty_type = str(sys.argv[3])
prior_type = str(sys.argv[4])
alter_type = str(sys.argv[5])
dataset_name = str(sys.argv[6])

classifer_name = "ADDT_nS_" + penalty_type + "_" + prior_type + "_" + alter_type 
...
classifier = aggregate.DDT(numOfLabels, penalty_type, prior_type, alter_type)
classifier.fit(X_train, y_train)
testErr = 1-classifier.score(X_test, y_test)
filename = 'eTS/each experiment, each train size/' + dataset_name + '-' + classifer_name + '-ex'+str(ex_index)+'tr'+str(train_index)+'.npy'
...
if (train_index == 4):
    ...
    filename = 'MD/each experiment, each train size/' + dataset_name + '-' + classifer_name + "-margin=log ratio" +  '-ex' + str(ex_index)+'tr'+str(train_index)+'.npy'
    ...
    filename = 'MD/each experiment, each train size/' + dataset_name + '-' + classifer_name + "-margin=average gap" +  '-ex' + str(ex_index)+'tr'+str(train_index)+'.npy'
    margins = np.concatenate((misClassifiedVector.reshape(test_size,1), margin_list.reshape(test_size,1)), axis=1)
    np.save(filename,margins)